...
Specialized databases, :py:class:`~pod_lca.impacts.ElectricityImpactsDatabase` and :py:class:`~pod_lca.impacts.EOLImpactsDatabase`, are inherited from the base class :py:class:`~pod_lca.impacts.ImpactsDatabase` to account for additional classifiers present in those databases (e.g., `region` and `technology type` used in `ElectricityImpactsDatabase`). 
