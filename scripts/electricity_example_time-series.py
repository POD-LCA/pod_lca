...
data_dict = {category:{} for category in IMPACT_CATEGOREIS}
...
for category in IMPACT_CATEGOREIS:
    ...
    graph.draw(data_dict[category], "Electricity Emissions over years", "Year", f"{category} ({IMPACT_CATEGOREIS[category]})")
    graph.show()
