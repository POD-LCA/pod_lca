...
for ferc_region in ferc_regions:
    ...
    for technology in technologies:
        ...
        tmp_1 = trading_matrix["FERC"][trading_matrix["FERC"]["import ferc region abbr"] == ferc_region].get(
            ["export_name", "fraction"]
        )
        tmp_2 = generation_mix[generation_mix["FuelCategory"] == technology].drop(
            ["FuelCategory", "Electricity"], axis=1, inplace=False
        )
        tmp_3 = pd.merge(tmp_1, tmp_2, left_on="export_name", right_on="Subregion", how="inner").drop(
            "export_name", axis=1
        )
        tmp_4 = tmp_3[(tmp_3["fraction"] > 0.0) & (tmp_3["Generation_Ratio"] > 0.0)]
        tmp_5 = pd.merge(
            tmp_4, trans_dist_grid_loss, left_on="Subregion", right_on="Balancing Authority Name", how="inner"
        ).drop("Subregion", axis=1)
        ...
        for index, row in tmp_6.iterrows():
            ...
            if data_row.empty:
                ...
                for impact_cat in impact_data_headers:
                    impact_val = (
                        electricity_impact[electricity_impact["Name"] == USLCI_entry][impact_cat] * row["multiplier"]
                    )
