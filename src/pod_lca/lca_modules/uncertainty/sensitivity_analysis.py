    ...
    def compute_sensitivity_of_param(
        ...
        **kwargs
        ...
        if "range" in kwargs:
            ...
            corr_options = [
                kwargs["options"][results.index(min(results))],
        ...
        for objective in ["min", "max"]:
            for group in groups:
                ...
                if "range" in group:
                    ...
                    if (base_val > range_lst[1]) or (base_val < range_lst[0]):
                        raise ValueError(
                            f"The range of {param} ({range_lst}) does not include the base value ({base_val})"
                        )
                    ...
                    for option in group["options"]:
                        ...
                        if (objective == "min" and impact_tmp <= ref_impact) or (
                            objective == "max" and impact_tmp >= ref_impact
                        ):
            ...
            if sensitivity_type == "relative":
                ...
                results[objective] = 100 * (impact_new - base_impact) / (base_impact + impact_new)
            else:
                raise ValueError(f"Invalid sensitivity type: {sensitivity_type}")
