...
class AR6Calculations(ARXCalculation):
                ...
                if greenhouse_gas == "CH4":
                    if ref_unit == "Wm-2ppb-1":
                        ...
                        radiative_efficiency += (
                            (indirect_factors["f1"] + indirect_factors["f2"])
                            * (molecular_weight_air_mean / molecular_weight)
                            * (10**9 / mass_atmosphere_total)
                        )  # FIXME
                    else:
    ...
    def get_radiative_forcing(
        cls, greenhouse_gas, at_year, cumulative=False, CH4_oxidation=False, alpha=0.75, convolution_time_step=0.01
    ):
