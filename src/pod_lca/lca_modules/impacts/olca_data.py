...
class openLCA:
    ...
    def get_category_in_process(categories, node, level, impact, qty, unit, conversion_map, max_levels=3):
        ...
        if level < max_levels:
            for child in node.childs:
                impact, qty, unit = openLCA.get_category_in_process(
                    categories, child, level + 1, impact, qty, unit, conversion_map, max_levels
                )
    ...
    def get_process_in_process(processes, node, level, impact, qty, unit, conversion_map, max_levels=3):
            ...
            if unit is None:
                ...
                if conversion_map is None:
                    ...
                    if node.provider.id in conversion_map:
                        conversion_factor_a = UNITS_MAP[node.product.ref_unit].convert_to(
                            UNITS_MAP[conversion_map[node.provider.id]["declared_unit"]]
                        )
                        conversion_factor_b = float(conversion_map[node.provider.id]["conversion_factor"]) / float(
                            conversion_map[node.provider.id]["declared_qty"]
                        )
                        conversion_factor_c = UNITS_MAP[conversion_map[node.provider.id]["converted_unit"]].convert_to(
                            unit
                        )
        ...
        if level < max_levels:
            for child in node.childs:
                impact, qty, unit = openLCA.get_process_in_process(
                    processes, child, level + 1, impact, qty, unit, conversion_map, max_levels
                )
    ...
    def generate_impacts_dir(client, process_list, impact_dict, impact_method, group_by=None):
        ...
        for process in tqdm(process_list):
            ...
            process_results = {
                ...
                "Process Type": process.process_type.name,
                ...
                for group in group_by:
                    ...
                    for impact_cat in impact_dict:
                        ...
                        if group["is_uuid"]:
                            ...
                            group_impact, ref_qty, ref_unit = openLCA.get_category_in_process(
                                ids, root, 0, impact=0, qty=0, unit=unit, conversion_map=conversion_map, max_levels=1
                            )
                        else:
