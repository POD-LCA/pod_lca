...
from ui.materials_screening_tool.item import Item
...
class Process(Item):
    ...
    def open_popup_process(self):
        ...
        cmd = lambda: self.create_process(
    ...
    def create_process(self, popup, name, qty, unit, stage, lca_data):
        ...
        if not process is None:
            ...
            item_id, text_item, text_id = Process.create_canvas_item(
                self, model_id, name, stage, qty, unit, self.color_process, tags=["process"]
            )
            slider_cmd = lambda x: Process.update_qty(self, item_id, x)
            slider, slider_data = Process.create_slider(
