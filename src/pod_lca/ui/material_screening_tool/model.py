...
class ModelMixin:
    ...
    def copy_model(self, source_model_id):
        ...
        connections_lst = self.restore_connections(
            state["connectors"][source_model_id], item_id_history, new_model_id
        )  # FIXME
        self.connectors[new_model_id] = connections_lst

        self.dependents[new_model_id], self.relationships[new_model_id] = self.restore_relationships(
            item_id_history, state["dependents"][source_model_id], state["relationships"][source_model_id]
        )  # TODO: test
        ...
        if file_path:
            ...
            if tmp_transportation_map:
                for entry in tmp_transportation_map:
                    ...
                    line = self.current_canvas.create_line(
                    ...
                    self.connectors[model_id].append(
                        {"line": line, "start_item": product_item_id, "end_item": transporter_item_id}
                    )
