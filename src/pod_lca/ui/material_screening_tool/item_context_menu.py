...
class ItemContextMenuMixin:
        ...
        if master.shift_pressed:
            ...
            if "item" in master.current_canvas.gettags(item):
                ...
                master.context_menu.add_command(
                    label="View unit impacts", command=lambda: cls.view_impacts(master, item)
                )
                ...
                master.context_menu.add_command(
                    label="Change life cycle stage", command=lambda: cls.update_life_cycle_stage(master, item)
                )
                if GUIInputManager.is_product(master.item_map[model_id][item]):
                    master.context_menu.add_command(
                        label="Set density", command=lambda: cls.set_product_density(master, item)
                    )
                master.context_menu.add_separator()

                slider_menu = Menu(master.context_menu, tearoff=False)
                slider_on = BooleanVar(value=slider._always_on)
                slider_menu.add_radiobutton(
                    label="On",
                    variable=slider_on,
                    value=True,
                    command=lambda: cls.set_slider_state(master, item, status="on"),
                )
                slider_menu.add_radiobutton(
                    label="Off",
                    variable=slider_on,
                    value=False,
                    command=lambda: cls.set_slider_state(master, item, status="off"),
                )
                slider_menu.add_separator()
                slider_menu.add_command(
                    label="Set slider properties", command=lambda: cls.set_slider_properties(master, item)
                )
    ...
    def change_qty(cls, master, item_id):
        ...
        qty = Popup._popup_input_field(
            popup,
            f"Qty (in {GUIInputManager.get_unit(item)}): ",
            validate_num=True,
            default_val=GUIInputManager.get_qty(item),
        )
    ...
    def set_product_density(cls, master, item_id):
        ...
        density = Popup._popup_input_field(
            popup, "Weight per unit product: ", validate_num=True, default_val=GUIInputManager.get_density(item)
        )
    ...
    def set_slider_properties(cls, master, item):
        ...
        qty_min = Popup._popup_input_field(
            popup, "qty slider min: ", validate_num=True, default_val=slider.cget("from")
        )
        qty_max = Popup._popup_input_field(popup, "qty slider max: ", validate_num=True, default_val=slider.cget("to"))
        qty_reolution = Popup._popup_input_field(
            popup, "qty slider resolution: ", validate_num=True, default_val=slider.cget("resolution")
        )
