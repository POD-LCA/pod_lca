...
class ConnectorsMixin:
    ...
    def on_start_connector(self, event):
        ...
        if start_item:
            ...
            if not self.shift_pressed:
                ...
                self.connector_data["line"] = self.current_canvas.create_line(
                    event.x,
                    event.y,
                    event.x,
                    event.y,
    ...
    def on_draw_connector(self, event):
        if not self.shift_pressed:
            if self.connector_data["line"] is not None:
                self.current_canvas.coords(
                    self.connector_data["line"],
                    self.connector_data["start_x"],
                    self.connector_data["start_y"],
                    event.x,
                    event.y,
                )
        else:
            ...
            if self.shift_pressed:
                ...
                if self.allow_to_connect(end_item) and not self.already_connected(start_item, end_item):
                    ...
                    self.connectors[model_id].append(
                        {"line": self.connector_data["line"], "start_item": start_item, "end_item": end_item}
                    )
                    ...
                    if connects_to_transport:
                        GUIInputManager.set_transported_product(
                            self, self.item_map[model_id][end_item], self.item_map[model_id][start_item]
                        )
                        self.update_plot()
    ...
    def delete_connection(self, start_item, end_item):
        ...
        for connector in self.connectors[model_id]:
            if connector["start_item"] == start_item and connector["end_item"] == end_item:
                ...
                if "product" in self.current_canvas.gettags(
                    connector["start_item"]
                ) and "process" in self.current_canvas.gettags(connector["end_item"]):
    ...
    def update_connectors(self, item):
        ...
        for connector in self.connectors[model_id]:
            start_item_id = (
                connector["start_item"] if isinstance(connector["start_item"], int) else connector["start_item"][0]
            )
    ...
    def restore_connections(self, connections, item_id_map, model_id):
        ...
        for connector in connections:
            ...
            smooth = True if self.connector_type == "spline" else False
            tmp_connector["line"] = self.models[model_id].create_line(
                0, 0, 0, 0, fill=self.connector_color, width=2, smooth=smooth, tag="connector"
            )
    ...
    def redraw_connections(self):
        ...
        for connector in self.connectors[model_id]:
            smooth = True if self.connector_type == "spline" else False
            connector["line"] = self.current_canvas.create_line(
                0, 0, 0, 0, fill=self.connector_color, width=2, smooth=smooth, tag="connector"
            )
            self.draw_connection(connector["start_item"], connector["end_item"], connector["line"])
