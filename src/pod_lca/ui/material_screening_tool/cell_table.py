...
class CellTable(Treeview):
    ...
    def import_data(self, hotspots=True):
        ...
        for product in GUIInputManager.get_all_products(self.GUI, self.model) + GUIInputManager.get_all_processes(
            self.GUI, self.model
        ):
            if hotspots:
            ...
            row_id = self.insert(
                "", END, iid=GUIInputManager.get_id(product), values=(self.get_obj_values(product)), tags=(tag,)
            )
    ...
    def edit_cell(self, event):
        ...
        if col_num == self.lc_stage_col:
            ...
            dropdown_combobox = Combobox(
                self.master, textvariable=combobox_var, values=dropdown_options, state="readonly"
            )
            dropdown_combobox.place(x=x, y=y + (2 * height), width=width)
            dropdown_combobox.focus()

            dropdown_combobox.bind(
                "<<ComboboxSelected>>", lambda event: self.update_lc_stage(event, dropdown_combobox, combobox_var)
            )
        ...
        if col_num == self.impact_col:
            ...
            dropdown_combobox.bind(
                "<<ComboboxSelected>>", lambda event: self.update_impact_data(event, dropdown_combobox, combobox_var)
            )
            dropdown_combobox.bind("<Escape>", lambda event: self.cancel_edit(event, dropdown_combobox))
    ...
    def on_enter(self, event, entry, selected_item, col_num):
        ...
        if self.headings[col_num - 1] in ["Name", "name"]:
            ...
            GUIInputManager.update_life_cycle_stage(
                self.GUI, self.GUI.item_map[self.model][int(selected_item)], new_value
            )
    ...
    def update_lc_stage(self, event, dropdown_combobox, combobox_var):
        ...
        GUIInputManager.update_life_cycle_stage(
            self.GUI, self.GUI.item_map[self.model][int(selected_item)], selected_stage
        )
