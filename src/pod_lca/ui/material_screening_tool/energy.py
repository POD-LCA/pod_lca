...
class EnergyProduct(Product):
    ...
    def open_popup_energy(self):
        ...
        cmd = lambda: self.create_energy_product(
    ...
    def create_energy_product(self, popup, name, qty, unit, stage, lca_data):
        ...
        product = GUIInputManager.create_energy(self.project, model_id, name, unit, float(qty), stage, lca_data)
        ...
        if not product is None:
            ...
            item_id, text_item, text_id = EnergyProduct.create_canvas_item(
                self, model_id, name, stage, qty, unit, self.color_energy, tags=["product", "energy"]
            )
            slider_cmd = lambda x: EnergyProduct.update_qty(self, item_id, x)
            slider, slider_data = EnergyProduct.create_slider(
