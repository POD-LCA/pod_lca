...
class MenubarMixin:
    ...
    def _update_setting(self, menubar):
        ...
        color_transport = Popup._popup_input_field(
            color_tab_frame, "Transportation process color", default_val=self.color_transport
        )
        color_energy = Popup._popup_input_field(color_tab_frame, "Energy product color", default_val=self.color_energy)
        color_emission = Popup._popup_input_field(
            color_tab_frame, "Emission product color", default_val=self.color_emission
        )
        color_waste = Popup._popup_input_field(color_tab_frame, "Waste product color", default_val=self.color_waste)
        cmd_color = lambda: self._apply_settings_colors(
            color_canvas.get(),
            color_connectors.get(),
            tag_colors={
                "product": color_product.get(),
        ...
        canvas_grid = Popup._popup_radio_buttons(
            grid_tab_frame,
            {"On": True, "Off": False},
            self.canvas_grid,
            cmd=lambda: Popup.update_entry_state(bool(int(canvas_grid.get())), grid_spacing),
        )
        grid_spacing = Popup._popup_input_field(
            grid_tab_frame, "Grid spacing", default_val=self.canvas_grid_size, validate_num=True
        )
        ...
        color_connectors = Popup._popup_input_field(
            connector_tab_frame, "Connector color", default_val=self.connector_color
        )
        Popup._popup_label(connector_tab_frame, "Connector type:", justify="left")
        connector_type = Popup._popup_radio_buttons(
            connector_tab_frame,
            {"straight": "straight", "elbow": "elbow", "spline": "spline"},
            self.connector_type,
            cmd=lambda: Popup.update_entry_state(connector_type.get() != "straight", connector_offset),
        )
        connector_offset = Popup._popup_input_field(
            connector_tab_frame, "Connector offset", default_val=self.connector_offset, validate_num=True
        )
        cmd_connectors = lambda: self._apply_settings_connectors(
            color_connectors.get(), connector_type.get(), connector_offset.get()
        )
    ...
    def import_database(self, menubar):
        ...
        browse_button = Button(
            popup, text="Select File", command=lambda: menubar._file_open_dialog(menubar, file_path, popup)
        )
    ...
    def add_custom_item(self, menubar):
        ...
        unit = Popup._popup_input_combo(
            popup, "Unit flow: ", GUIInputManager.get_all_units_list(self.project), default_entry=0
        )
    ...
    def create_analysis_menu(self, menubar):
        ...
        menu_hotspot.add_radiobutton(
            label="On", variable=self.hotspot_on_off, value=True, command=lambda: self.show_hotspots()
        )
        menu_hotspot.add_radiobutton(
            label="Off", variable=self.hotspot_on_off, value=False, command=lambda: self.clear_hotspots()
        )
        menu_hotspot.add_separator()
        self.hotspot_impact_cat = StringVar(value="GWP")
        menu_hotspot.add_radiobutton(
            label="GWP", variable=self.hotspot_impact_cat, value="GWP", command=lambda: self.show_hotspots()
        )
        menu_hotspot.add_radiobutton(
            label="AP", variable=self.hotspot_impact_cat, value="AP", command=lambda: self.show_hotspots()
        )
        menu_hotspot.add_radiobutton(
            label="EP", variable=self.hotspot_impact_cat, value="EP", command=lambda: self.show_hotspots()
        )
        menu_hotspot.add_radiobutton(
            label="ODP", variable=self.hotspot_impact_cat, value="ODP", command=lambda: self.show_hotspots()
        )
        menu_hotspot.add_radiobutton(
            label="SFP", variable=self.hotspot_impact_cat, value="SFP", command=lambda: self.show_hotspots()
        )
    ...
    def open_DQA_view(self, menubar):
        ...
        label = Label(
            dropdown_frame, bg=self.plotter_bg_color, fg="white", text="Model", font=("Helvetica", 12, "bold")
        )
        ...
        model_dropdown.bind(
            "<<ComboboxSelected>>", lambda x: self.create_score_panel(score_panel_frame, x.widget.get())
        )
    ...
    def create_score_panel(self, frame, model_name):
        ...
        for obj in pedigree_score_dict:
            ...
            for indicator in indicators:
                ...
                dropdown.current(
                    score_list.index(GUIInputManager.get_pedigree_score(pedigree_score_dict[obj], indicator))
                )
                dropdown.bind(
                    "<<ComboboxSelected>>",
                    lambda x, obj=obj, indicator=indicator: self.update_DQS(
                        x,
                        obj,
                        pedigree_score_dict,
                        indicator,
