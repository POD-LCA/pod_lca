...
class HotspotMixins:
    ...
    def show_impact_bubble(self, item_id, impact_category, max_impact, k=0.5):
        ...
        oval = self.current_canvas.create_oval(
            x0, y0, x1, y1, outline="black", fill="red", width=2, tag="impact_bubble"
        )
        label = self.current_canvas.create_text(
            coords[2], coords[1], text=impact_str, anchor="center", tag="impact_bubble"
        )
