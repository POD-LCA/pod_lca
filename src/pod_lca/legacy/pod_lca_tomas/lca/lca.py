...
from legacy.pod_lca_tomas.lca.process import Process
from legacy.pod_lca_tomas.lca.process import Flow
...
class LCA(object):
    ...
    def read_ecoinvent_lcia(self):
        with open(os.path.join(legacy.pod_lca_tomas.DATA, "maps", "ecoinvent_lcia_categories_map.json"), "r") as fp:
    ...
    def find_upstream_impacts_sequential(self, processes_, amounts, units):
        ...
        while len(temp_processes) > 0:
            ...
            out_elm = [
                e for e in process["exchanges"] if not e["isInput"] and e["flow"]["flowType"] == "ELEMENTARY_FLOW"
            ]
            out_was = [e for e in process["exchanges"] if not e["isInput"] and e["flow"]["flowType"] == "WASTE_FLOW"]
            input_exchanges = [
                e for e in process["exchanges"] if e["isInput"] and e["flow"]["flowType"] != "ELEMENTARY_FLOW"
            ]
            output_exchanges = [e for e in process["exchanges"] if not e["isInput"]]
