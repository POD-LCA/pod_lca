...
custom_impact_database.set_data(r"src/pod_lca/data/impacts_podlca_data.csv", additional_headers="Mineral Carbonation Potential")
project.set_impact_database(custom_impact_database)

my_model = project.add_model("CLT_01")

# Biogenic C
hardwood_bark = my_model.add_product(
    name="hardwood bark",
    stage="A1",
    qty=2,
    unit=KILOGRAM,
    impacts_from="Bark, hardwood, average, at forest road,  NE-NC",
)
print(hardwood_bark.get_impacts())
print("adjusted GWP:", hardwood_bark.get_impacts().get_adjusted_GWP())

# Mineral C
aggregate = my_model.add_product(
    name="aggregate", stage="A1", qty=1, unit=KILOGRAM, impacts_from='Aggregate; Vulcan Materials; 3/4" x #4 Gravel'
)
