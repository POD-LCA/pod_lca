...
test_uuid_list = ['d1b276b6-52ab-40e8-87bc-4b03fa60526f',
...
results = openLCA.generate_impacts_dir( openLCA_client, test_process_list, 
...
for uuid in tqdm(test_dict):
    test = test_dict[uuid]
    output_dict[uuid] = { 'Category':test['Category'],
                          'Name': test['Name'], 
                          'UUID': test['UUID'], 
    ...
    for header in ['electricity_', 'nonrenewable fuel combustion_', 'renewable fuel combustion_']:
        for impact_category in impact_categories:
            ...
            if abs(float(test[header + impact_category])) > 0.0:
                diff = abs(float(test[header + impact_category]) - results[test['UUID']][header + impact_category]) / float(test[header + impact_category])
            else:
                diff = 0.0
        ...
        if abs(float(test[header + 'qty'])) > 0.0:
            diff = abs(float(test[header + 'qty']) - results[test['UUID']][header + 'qty']) / float(test[header + 'qty'])
        else:
            diff = 0.0
