...
def run_test_files(test_file_path, output_csv_path):
    ...
    for _, row in test_inputs.iterrows():
        ...
        try:
            ...
            try:
                domestic_impact = truck_1.get_impact_domestic().get_record_dict()
            except Exception:
                domestic_impact = None

            try:
                domestic_mode_impact = truck_1.get_mode_domestic().get_impact().get_record_dict()
            except Exception:
                domestic_mode_impact = None

            try:
                foreign_impact = truck_1.get_impact_foreign().get_record_dict()
            except Exception:
                foreign_impact = None

            try:
                foreign_mode_impact = truck_1.get_mode_foreign().get_impact().get_record_dict()
            except Exception:
...
if __name__ == "__main__":
    ...
    output_csv_path = r"tests\transportation_test_report.csv"
