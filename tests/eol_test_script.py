...
def test_eol():
    ...
    test_building = Building.build(
        name="Test Building", type="Commercial", location=my_land_plot, built_year=2025, geometry=None
    ...
    for test_name in tests_dict:
        ...
        test_component = BuildingComponent.create(
            name=test_name, materials=[None]
        ...
        impact_dict = {
            "C2": Impacts.from_parent(test_component),
            ...
            for impact_cat in impact_categories:
                if impact_cat + "_" + lc_stage in test_data_dict[test_name]:
                    ...
                    output_dict[test_name][
                        impact_cat + "(" + impact_categories[impact_cat] + lc_stage + ")" + " Python tool"
                    ] = computed
                    output_dict[test_name][
                        impact_cat + "(" + impact_categories[impact_cat] + lc_stage + ")" + " Excel tool"
                    ] = reference
    ...
    print(f">>>> EOL tests passed: {tests_passed} of {tests_total}")
